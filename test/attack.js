...
describe("Attack", function () {
  it("Should empty the balance of the good contract", async function () {
    ...
    let balanceETHGood = await ethers.provider.getBalance(goodContract.address);
    expect(balanceETHGood).to.equal(parseEther("10"));

    console.log(`The balance of goodContract before exploit is ${balanceETHGood}`)
    ...
    console.log(`The balance of innocent contract after the exploit is ${balanceETH}`)
    

    // Balance of BadContract is now 11 ETH (10 ETH stolen + 1 ETH from attacker)
    balanceETHBad = await ethers.provider.getBalance(badContract.address);
    expect(balanceETHBad).to.equal(parseEther("11"));
    console.log(`The balance of badContractt before exploit is ${balanceETHBad}`)
